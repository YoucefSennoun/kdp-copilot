...
> **Revision 2 note:** after that fix, live suggestions still included things like *Antifragile*, *How Countries Go Broke*, *Principles: Life and Work*, *The Intelligent Investor*, and *The Runaway Bunny*. These aren't just "high-content" in the generic sense — every one of them is the **exact title of a specific, already-published, single-author book that already dominates that exact search term.** That's a distinct, worse problem than expertise-gated nonfiction in general: it's not that a new entrant would struggle to compete, it's that the buyer typing that phrase is looking for *that specific existing book*, so no new title (of any content type) can capture that demand, no matter how the numbers look. See Gap F and the Layer 0 addition to Phase 1.5 below. (Note: I couldn't re-inspect your current local code for this pass — the public repo on `master` hasn't changed since my first read, so whatever your agent built since is presumably still local/uncommitted. This addition is written against the original architecture and your reported symptoms; file/function names may have moved.)
...
## 2. The gaps that block your exact niche criteria
...
### Gap F — Nothing checks whether the suggested keyword *is* an existing, dominant book
`buildExpansionPrompt()` asks Gemini for niches "an independent author can actually win," but never tells it to reject a suggestion if the keyword itself is the title of a real, already-published, single-author work. The model clearly knows these are real books (it's not confusing them for anything else) — it's just never told that matters. Nothing downstream catches this either: there's no signal anywhere in `scoring.js` for "one listing holds a wildly outsized share of this keyword's demand," which is the measurable fingerprint of a branded/single-work-dominated search term versus a genuinely open category. `computeTopConcentration()` counts how many listings clear a 300-review floor — it doesn't check whether *one* listing towers over everyone else combined, which is the actual shape of "The Intelligent Investor" or "Principles."
...
metrics.leaderDominanceRatio     // top listing's reviewCount / totalReviews — a "branded/existing-title" tell
qualifies: {
  bsr: boolean,
  listings: boolean,
  volume: boolean,
...
### Phase 1.5 — KDP-Publishable Content-Type Filter (fixes Gaps E & F) — do this before scaling up Discovery Mode
This is what your latest testing round exposed, and it's cheap to fix at the source before it gets expensive to fix downstream. Three layers — Layer 0 is the new addition for Gap F, Layers 1–2 are unchanged from the first revision:

**Layer 0 — reject exact-existing-book suggestions before they're even saved (do this first, it's the direct fix for the *Antifragile*/*Intelligent Investor* examples):**
- Add an explicit hard rule to `buildExpansionPrompt()`: *"Never suggest a keyword that is the exact or near-exact title of a real, previously published, identifiable book — fiction, memoir, or expert-authored nonfiction (e.g. do not suggest things like 'Antifragile', 'Principles: Life and Work', 'The Intelligent Investor', 'The Runaway Bunny'). If a candidate niche resembles an existing title, discard it and propose a genuinely generic, unbranded phrase instead."* Naming a few real offending examples directly in the prompt (few-shot negative examples) is a stronger signal to the model than an abstract instruction, and you already have a live list of exactly the failures to feed back in.
- Add a synchronous filter in `expandNicheSeeds()` (`background/ai.js`) that runs on the AI's own returned array **before** anything is saved to storage: reject any suggestion whose `keyword` (or `titleIdea`) trips your `HIGH_CONTENT_SIGNALS` list from Layer 2 below, or whose `why`/`category` text contains tells like "by [an author]", "bestseller", "classic". This needs no scraping — it's a pure string check on data you already have the instant the AI responds, so it can run at the Suggestions stage, before "0 scraped" even changes.
- **Debug note, separate from content filtering:** several of your pasted rows share byte-identical score/percentage tuples across completely unrelated titles (e.g. multiple different keywords all showing `64.044 / 40% / 10% / 49% / 60%`). That shouldn't happen if each suggestion is genuinely scored on its own — worth having the agent check whether the Suggestions-stage "score" is a real per-item computation or a shared/cached object being reused across array entries (a common bug shape: mapping over the AI's array but assigning from one shared score variable instead of recomputing per item).

**Layer 1 — stop generating high-content suggestions in general (cheapest fix, do first):**
- Rewrite `buildExpansionPrompt()` in `background/ai.js` to explicitly scope Gemini's suggestions to what you can actually publish: low-content (journals, planners, logbooks, notebooks, diaries, calendars, trackers), medium-content (coloring books, activity/puzzle/guided-workbook books), personalized/name-variant books, and researched-and-compiled guides that don't require professional credentials. Explicitly instruct it to exclude novels, memoirs, children's picture books, and nonfiction requiring subject-matter expertise (medical, biological, scientific, technical, legal, academic, financial/investing). Add `contentType` as a required field in the JSON schema the prompt already asks for, so every AI-suggested niche is self-tagged at generation time instead of needing to be classified after the fact.
...
**Layer 2 — classify and gate anything that still slips through** (organic autocomplete expansion, manually typed seeds, ambiguous AI suggestions, and anything Layer 0/1 miss):
- Add `src/lib/content-type.js` with two curated word lists — `LOW_MEDIUM_SIGNALS` and `HIGH_CONTENT_SIGNALS` — and a cheap rule-based classifier that runs immediately on SERP data, no extra scraping required:
  ...
  - **Leader dominance ratio (the direct fix for Gap F, and the one signal that works regardless of what the AI did):** add `metrics.leaderDominanceRatio = topListing.reviewCount / metrics.totalReviews` to `computeListingsStats()`. A keyword where one listing holds, say, over 50–60% of all reviews in the sample is a branded/single-work-dominated term almost by definition — hard-exclude it from `qualifies.contentType` regardless of everything else, since this is true whether the dominant listing is "The Intelligent Investor" or just an unusually entrenched competitor.
- For keywords that already clear your BSR/listings gates but land in a genuinely ambiguous zone (e.g. "diabetes recipe journal" vs. "diabetes clinical management guide" — regex can't reliably tell these apart), send title + category breadcrumb + formats to Gemini for a final call: `{contentType, requiresExpertise, isExistingTitle, reasoning}`. Only run this AI-confirmation step on the shortlist that already passed the cheap rule-based filter and your BSR/listings gates — the same cost-control pattern already used for BSR enrichment in Phase 2.
- **Acceptance:** run the pipeline against a deliberately mixed test batch (include a known medical-nonfiction seed, a known fiction seed, and at least one known existing-bestseller title on purpose — "the intelligent investor" itself is now a permanent regression-test case) and confirm none of them reach the Niche Finder tab; confirm known journal/planner/coloring-book/personalized examples pass through untouched.
...
3. **Phase 1.5 next, before scaling anything up** — Layer 0 (the few-shot negative examples + synchronous post-generation filter) is the direct fix for the *Antifragile*/*Intelligent Investor*-style failures and should go in first; Layer 1 (prompt rewrite + fallback-ranker weighting) stops most of the general high-content contamination; Layer 2 (the rule-based + AI classifier, including `leaderDominanceRatio`) is the scrape-dependent safety net and can follow once you see how much Layers 0–1 fix on their own.
...
## 7. Implementation check-in (reviewed the pushed "new version" folder against this plan)

Good news first: this is a faithful, competent build of most of the plan. Confirmed present and matching spec: `getTotalResultsCount()` in `serp-parser.js` (Phase 1, same selectors this plan suggested), `computeBsrStats()`/`bestSubcategoryBsr` in `scoring.js` (Phase 2), `computeDemandProxyScore()` in the new `lib/proxy.js` (Phase 3), `discovery.js` + `discovery-parser.js` + a working `chrome.alarms` listener (Phase 4), a `computeQualifies()` 4-gate function and a `views/niches.js` Niche Finder tab that correctly filters on `bsr && listings && volume && contentType` (Phase 5), and the `content-type.js` classifier (Phase 1.5 Layer 2) is genuinely well-built — better than the original spec in places (e.g. it correctly avoids excluding "clinical trial logbook" just because "clinical" appears, by checking whether a strong low-content signal coexists).

Two specific, narrow bugs explain exactly what you're still seeing — both are small, targeted patches, not redesigns:

**Bug 1 — the content-type classification is computed but never enforced.** In `handleExpandSeed()` (`background/index.js`), every AI-suggested niche is correctly run through `classifyContentType({ keyword })` and the result is stored in `metrics.contentType` — but nothing then filters `records` based on it before `putKeywords(records)` saves them. The check runs; its answer is just never acted on. On top of that, the classifier is lexical (title-regex + category + Kindle-format), so a title like "the intelligent investor" or "antifragile" — which contains no fiction/expertise keyword at all — comes back `contentType: 'unknown'` rather than `'high-content-excluded'`, and `computeSuggestionQualifies()` treats `'unknown'` as passing (`excluded` only triggers on an explicit `'high-content-excluded'` or `requiresExpertise`). No amount of prompt tuning fixes this specific failure mode, because it's not a lexical pattern — it's "is this a real existing book," which only world-knowledge can answer.
- **Fix A (small, do first):** in the `records = suggestions.map(...)` block, actually drop suggestions where `ct.contentType === 'high-content-excluded'` **or** `ct.contentType === 'unknown'` — at the pre-scrape stage there's no other signal coming to rescue an `'unknown'` later, so treat it as "not cleared" rather than "innocent until proven guilty." (This is a `.filter()` before the `.map()`, or filtering the `records` array before `putKeywords`.)
- **Fix B (targeted, catches this exact failure class):** add `isExistingTitle: boolean` to the JSON schema in `buildExpansionPrompt()`, with an explicit instruction: *"true if this keyword is the exact or near-exact title of a real, previously published, identifiable book you recognize."* Reject any suggestion where `isExistingTitle === true`. Directly asking the model to self-report this one fact is far more reliable than hoping a general "don't suggest existing titles" instruction wins out against Gemini's strong training-data association with famous titles like these.

**Bug 2 — this is *also* the root cause of the duplicate-score-tuple pattern.** `deriveSuggestionProxy()` (`lib/proxy.js`) looks up each AI-suggested keyword's position inside the *seed's* shared autocomplete corpus (one alphabet-soup run per batch, reused across all ~12 suggestions — a reasonable cost optimization). But Gemini-generated phrasings ("how countries go broke: the big cycle") rarely appear verbatim in that corpus, so `bestPosition()` returns `null` for both engines on most suggestions. When that happens, `posSignal` and `crossSignal` both collapse to `0`, and the formula reduces to `depthSignal * 0.25` — where `depthSignal` is just `seedProxyScore`, **the same number for every suggestion in the batch**. That's not a caching bug, it's a real degenerate case: any two AI suggestions that both miss the corpus get the identical proxy score, and since `computeCompetition()`/`computeMargin()` are *also* structurally near-constant pre-scrape (`sampleSize` is hardcoded to `0` in the suggestion record, so every competition sub-factor except the flat price term is `0`), the entire visible score/percentage row collapses to the same tuple for unrelated titles.
- **Fix:** when a suggestion misses the shared corpus entirely, run one direct, cheap autocomplete lookup for that specific keyword (12 suggestions × 1 extra fetch each is trivial next to a full scrape) instead of falling back to the diluted seed-wide constant — or, more honestly, leave `demandProxyScore` as `null`/"not yet measured" for that suggestion rather than showing a number that looks precise but isn't. Separately, consider not running the full `scoreKeyword()` opportunity formula at the pre-scrape Suggestions stage at all — with `sampleSize: 0`, two of its three components are constants by construction, so the resulting "score" implies a precision the data doesn't support. Showing the proxy estimate directly, clearly labeled as a pre-scan estimate, would be more honest than a fabricated-looking 64.044.
